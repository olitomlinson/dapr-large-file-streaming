#!/usr/bin/env python3
"""
Binary File Transfer Test with Memory Monitoring
Tests binary file streaming through Dapr with detailed memory monitoring of both sidecars
"""
...
SIZE_MB = 100  # 100 MB binary payload
...
async def run_binary_test():
    """Run binary transfer and monitor memory"""
    print("=" * 80)
    print("Binary File Transfer Test with Memory Monitoring")
    print("=" * 80)
    print()
    print(f"Configuration:")
    print(f"  Payload size: {SIZE_MB} MB")
    print(f"  Chunk size: {CHUNK_SIZE:,} bytes (1 MB)")
    print(f"  Expected chunks: {SIZE_MB}")
    ...
    print(f"=== Starting Binary Transfer ===")
    ...
    if memory_log:
        ...
        if caller_increase < 10:
            print("✓ EXCELLENT: Memory increase is minimal (<10 MiB)")
            print("  Binary streaming is working perfectly!")
        elif caller_increase < 30:
            print("✓ VERY GOOD: Memory increase is very low (<30 MiB)")
            print("  Binary streaming is highly efficient")
        else:
            print("⚠ WARNING: Memory increase is higher than expected (>30 MiB)")
        ...
        # Sample every 1 second for 100MB test
        sample_interval = 1.0
        next_sample = 0
    ...
    log_file = '/tmp/binary_memory_log.json'
