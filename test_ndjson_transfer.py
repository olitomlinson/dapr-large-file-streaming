#!/usr/bin/env python3
"""
Large NDJSON Document Transfer Test
Tests large NDJSON (newline-delimited JSON) document transfer with detailed memory monitoring
of both Dapr sidecars. NDJSON is more memory-efficient than JSON arrays for streaming.
"""
...
NUM_RECORDS = 500000  # 500k records (generates ~140-150MB NDJSON)
RECORDS_PER_CHUNK = 100  # Stream in small chunks
PROXY_URL = "http://localhost:8001/test-ndjson-transfer"
...
async def run_ndjson_test():
    """Run large NDJSON transfer and monitor memory"""
    print("=" * 80)
    print("Large NDJSON Document Transfer Test with Memory Monitoring")
    print("=" * 80)
    print()
    ...
    try:
        subprocess.run(
            ["docker", "exec", "dapr-large-file-streaming-chunk-receiver-1",
             "rm", "-f", "/tmp/received_ndjson.ndjson"],
            capture_output=True,
            timeout=5
        )
    except:
    ...
    print(f"=== Starting NDJSON Transfer ===")
    ...
    try:
        ...
        # Verify file and count lines
        print("=== File Verification ===")
        try:
            # Check file size
            file_check = subprocess.run(
                ["docker", "exec", "dapr-large-file-streaming-chunk-receiver-1",
                 "stat", "-c%s", "/tmp/received_ndjson.ndjson"],
                capture_output=True,
                text=True,
                timeout=5
            )
    ...
    if memory_log:
        ...
        # Estimate NDJSON size
        estimated_ndjson_size_mb = file_size / (1024 * 1024) if 'file_size' in locals() else 0
        ...
        if estimated_ndjson_size_mb > 0:
            print(f"  Percentage of NDJSON size: {(caller_increase / estimated_ndjson_size_mb) * 100:.2f}%")
        print()
        ...
        if estimated_ndjson_size_mb > 0:
            print(f"  Percentage of NDJSON size: {(receiver_increase / estimated_ndjson_size_mb) * 100:.2f}%")
        print()

        # Calculate what would happen WITH retry policy
        if estimated_ndjson_size_mb > 0:
            expected_with_retry = estimated_ndjson_size_mb * 2.2  # 2.2x buffering observed

            print("Expected memory with DIFFERENT configurations:")
            print(f"  WITH retry policy (hypothetical): ~{expected_with_retry:.0f} MiB ({estimated_ndjson_size_mb:.0f}MB × 2.2)")
        ...
        if caller_increase < 30:
            print("✓ EXCELLENT: Memory increase is minimal (<30 MiB)")
            print("  NDJSON streaming is working perfectly with extremely low overhead!")
        elif caller_increase < 50:
            print("✓ VERY GOOD: Memory increase is very low (<50 MiB)")
            print("  NDJSON streaming is highly efficient")
    ...
    log_file = '/tmp/ndjson_memory_log.json'
    with open(log_file, 'w') as f:
        json.dump({
            ...
            'test_config': {
                ...
                'ndjson_size_bytes': file_size if 'file_size' in locals() else 0
            }
        }, f, indent=2)
